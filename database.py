...
class Database:
    ...
    def add_comment(self, comment, name, vote, event_id):
        ...
        #cursor = psycopg2.connect("dbname=suppliers user=postgres password=utku")
        query = """INSERT INTO "COMMENT" (comment_id, comment, name, vote, event_id) VALUES ((%s), (%s), (%s), (%s), (%s))"""
        cursor.execute(query, (comment_key, comment, name, vote, event_id))
        connection.commit()
        return comment_key 
    ...
    def add_event(self, title, date, location):
        with dbapi2.connect(self.dbfile) as connection:
            ...
            #cursor = psycopg2.connect("dbname=suppliers user=postgres password=utku")
            query = """INSERT INTO "EVENTS" (event_id, title, date, location) VALUES ((%s), (%s), (%s), (%s))"""
    ...
    def add_club(self, club):
        with dbapi2.connect(self.dbfile) as connection:
            ...
            #cursor = psycopg2.connect("dbname=suppliers user=postgres password=utku")
            query = """INSERT INTO "CLUBS" VALUES (club.club_id, club.name, club.founder, club.number_member, club.email, club.password)"""
            cursor.execute(query, (club_user_key, club.name, club.founder, club.number_member, club.email, club.password))
            connection.commit()
    ...
    def get_clubs(self):
        ...
        for club_id, name, founder, number_member, email, password in cursor:
            clubs.append((club_key, Club(club_id, name, founder, number_member, email, password)))
            club_key = club_key + 1
        return clubs
    ...
    def do_register_club(self, name, founder, number_member, email, password):
         with dbapi2.connect(self.dbfile) as connection:
                ...
                global club_user_key 
                club_user_key = club_user_key + 1 
                query = """INSERT INTO "CLUBS" (club_id, name, founder, number_member, email, password) VALUES ((%s), (%s), (%s), (%s), (%s), (%s))"""
                cursor.execute(query, (club_user_key, name, founder, number_member, email, password)) 
                connection.commit()
    ...
    def do_register(self, firstname, surname, email, department, password):
         with dbapi2.connect(self.dbfile) as connection:
                ...
                cursor.execute("""INSERT INTO "STUDENT" (student_id, firstname, surname, email, department, password) VALUES ((%s), (%s), (%s), (%s), (%s), (%s))""",
                    (student_user_key, firstname, surname, email, department, password)) 
